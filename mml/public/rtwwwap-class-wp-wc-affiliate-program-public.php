...
function rtwwwap_member_redirect_successful()
{
	...
	function rtwwwap_become_affiliate()
	{
		...
		if( $rtwwwap_aff_approved == 1 ){
			...
			$rtwwwap_subject 		= esc_html__( $rtwwwap_user_email.' Request to become an affiliate', 'rtwwwap-wp-wc-affiliate-program' );


			$rtwwwap_message 		= sprintf( '%s', esc_html__( $rtwwwap_user_name.' is Requested to be an affiliate of your site ', 'rtwwwap-wp-wc-affiliate-program' ));
			...
			if( !empty( $rtwwwap_prods ) ){
				...
				foreach( $rtwwwap_prods as $rtwwwap_key => $rtwwwap_value ){
					...
					if(RTWWWAP_IS_Easy == 1 && $rtwwwap_affilaite_template == 2){
						...
						$rtwwwap_html .= 				'<p class="rtwwwap-product-price">'.$rtwwwap_currency_sym.$rtwwwap_prod_price->price.'</p>';
						$rtwwwap_html .= 			'</div>';
							...
							<div class="rtwwap-bottom">
								<div class="rtwwwap-prdct-name rtwwwap-prdct-box-primary">
									...
									<div class= "rtwwwap-card-price">
										<span>'.$rtwwwap_prod_price->price.'</span>
	...
	*/
	function rtwwwap_generate_csv_callback(){
		...
		if ( $rtwwwap_check_ajax ){
			...
			foreach( $rtwwwap_products->posts as $rtwwwap_key => $rtwwwap_value ){
				...
				$rtwwwap_excel_obj->getActiveSheet()->setCellValue( 'F'.$rtwwwap_counter, esc_html( $rtwwwap_prod_price->get_price() ) );
				$rtwwwap_excel_obj->getActiveSheet()->getStyle( 'F'.$rtwwwap_counter)->getAlignment()->setWrapText(true);
				$rtwwwap_excel_obj->getActiveSheet()->setCellValue( 'G'.$rtwwwap_counter, esc_html( $rtwwwap_prod_price->get_price()  ) );
	...
	function rtwwwap_send_rqst_callback($withdrawal_amount, $rtwwwap_user_id){
		...
		$rtwwwap_message 		= "A new withdrawal request is generated of amount :".$rtwwwap_currency_sym.$withdrawal_amount;
	...
	function rtwwwap_create_signup_referral_from_woo($rtwwwap_user_id)
	{
		...
		// {	
			...
			$rtwwwap_html = 
				'<div class="rtwwwap_email_wrapper">
					<div class="rtwwwap_email_content"> Thank you for registering. </div>
					<div class="rtwwwap_email_content">You are now member of our team.</div>
					<p>For any more questions do not hesitate to send us an email.</p>
					<h3>Sincerely,</h3><h2>Our Team</h2>
				</div>';
				$rtwwwap_from 	= esc_html( get_bloginfo( 'admin_email' ) );
				$rtwwwap_user_name 	= esc_html( get_bloginfo( 'user_name' ) );
				$rtwwwap_subject = esc_html__( "WELCOME TO THE Our MLM SYSTEM", 'rtwwwap-wp-wc-affiliate-program' );
				$rtwwwap_headers[] 		= 'Content-Type: text/html; charset=utf-8';
